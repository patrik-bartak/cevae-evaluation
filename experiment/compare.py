...
def run_model(model: CausalMethod, score_functions: List[Callable[[List[float], List[float]], float]],
    ...
    if train_test_all_data:
        ...
        losses_dict = model.train(model_all_proxies_train, model_all_y_train, model_all_t_train,
                                  x_test=model_all_proxies_train, y_test=model_all_y_train, t_test=model_all_t_train)
    else:
        ...
        ite_test_truth = X_test['treatment_effect'].to_numpy()
        print(len(ite_test_truth))
        ite_train_truth = X_train['treatment_effect'].to_numpy()
        print(len(ite_train_truth))
        losses_dict = model.train(model_proxies_train, model_y_train, model_t_train,
                                  x_test=model_proxies_test, y_test=model_y_test, t_test=model_t_test,
                                  ite_test_truth=ite_test_truth, ite_train_truth=ite_train_truth)
    ...
    results_ground_truth = results_ground_truth.to_numpy()
    assert np.shape(results_ground_truth) == np.shape(results), f"Test metric shape mismatch: {np.shape(results_ground_truth)} vs {np.shape(results)}"
    return [score_function(results_ground_truth, results) for score_function in score_functions], losses_dict
